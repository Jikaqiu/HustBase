...
const RC CreateFile(const char *fileName)
{
	int fd;
	char *bitmap;
	PF_FileSubHeader *fileSubHeader;//页尾 已分配页数
	...
	Page page;//页面
	memset(&page,0,PF_PAGESIZE);//全置0
	bitmap=page.pData+(int)PF_FILESUBHDR_SIZE;//位图
	fileSubHeader=(PF_FileSubHeader *)page.pData;//强制类型转换
	fileSubHeader->nAllocatedPages=1;//第0页已分配
	bitmap[0]|=0x01;//0号页已用
...
{
	int fd;
	PF_FileHandle *pfilehandle=fileHandle;//文件句柄
	RC tmp;
	...
	pfilehandle->pHdrFrame->bDirty=false;//头帧 FALSE表示未被修改
	pfilehandle->pHdrFrame->pinCount=1;//访问当前页面线程数
	pfilehandle->pHdrFrame->accTime=clock();//页面最后访问时间
	pfilehandle->pHdrFrame->fileDesc=fd;//文件描述符
	pfilehandle->pHdrFrame->fileName=fileName;//文件名
	if(_lseek(fd,0,SEEK_SET)==-1){
	...
	pfilehandle->pHdrPage=&(pfilehandle->pHdrFrame->page);//头页
	pfilehandle->pBitmap=pfilehandle->pHdrPage->pData+PF_FILESUBHDR_SIZE;//位图
...
const RC AllocatePage(PF_FileHandle *fileHandle,PF_PageHandle *pageHandle)
{
	...
	if((fileHandle->pFileSubHeader->nAllocatedPages)<=(fileHandle->pFileSubHeader->pageCount)){
		for(i=0;i<=fileHandle->pFileSubHeader->pageCount;i++){
			byte=i/8;//字节
			bit=i%8;//位
			if(((fileHandle->pBitmap[byte])&(1<<bit))==0){//有空页
